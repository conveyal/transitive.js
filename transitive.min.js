...
function Display(t){this.labelZoomThreshold=.75,this.xScale=d3.scale.linear().domain([-2,2]),this.yScale=d3.scale.linear().domain([-2,2]),this.zoom=d3.behavior.zoom().scaleExtent([.25,4]),this.svg=d3.select(t).append("svg").append("g").call(this.zoom),this.svg.append("rect").attr("class","overlay"),this.setElement(t)}var d3=require("d3");module.exports=Display,Display.prototype.setElement=function(t){var e=t.clientWidth,i=t.clientHeight;this.xScale.range([0,e]),this.yScale.range([i,0]),this.zoom.x(this.xScale).y(this.yScale),this.svg.attr("width",e).attr("height",i),this.svg.select("rect").attr("width",e).attr("height",i)};
});
...
function Pattern(t){for(var e in t)"stops"!==e&&(this[e]=t[e]);this.stops=[],this.graphEdges=[]}var d3=require("d3");module.exports=Pattern,Pattern.prototype.draw=function(t,e){this.svgGroup=t.svg.append("g"),this.line=d3.svg.line().x(function(s,r){var o,i=s.x;return o=0===r?t.xScale(i)+e*s.outEdge.vector.x:r===this.stops.length-1?t.xScale(i)-e*s.inEdge.vector.x:t.xScale(i),s.offsetX&&(o-=s.offsetX),o}.bind(this)).y(function(s,r){var o,i=s.y;return o=0===r?t.yScale(i)-e*s.outEdge.vector.y:r===this.stops.length-1?t.yScale(i)+e*s.inEdge.vector.y:t.yScale(i),s.offsetY&&(o+=s.offsetY),o}.bind(this)).interpolate("linear"),this.lineGraph=this.svgGroup.append("path").attr("class","transitive-line"),this.stopSvgGroups=this.svgGroup.selectAll(".transitive-stop").data(this.getStopData()).enter().append("g");var s=d3.behavior.drag().on("dragstart",function(){d3.event.sourceEvent.stopPropagation()}).on("drag",function(e){e.stop.graphVertex&&(e.stop.graphVertex.moveTo(t.xScale.invert(d3.event.sourceEvent.pageX-t.offsetLeft),t.yScale.invert(d3.event.sourceEvent.pageY-t.offsetTop)),t.refreshAll())});this.stopSvgGroups.append("circle").attr("class","transitive-stop-circle").call(s),this.stopSvgGroups.append("text").attr("id",function(t){return"transitive-stop-label-"+t.stop.getId()}).text(function(t){return t.stop.stop_name}).attr("class","transitive-stop-label")},Pattern.prototype.refresh=function(t){var e=this.lineGraph.style("stroke-width");this.lineWidth=parseInt(e.substring(0,e.length-2),10);var s=this.getStopData();this.lineGraph.attr("d",this.line(s)),this.stopSvgGroups.data(s),this.stopSvgGroups.attr("transform",function(e){var s=t.xScale(e.x)-e.offsetX,r=t.yScale(e.y)+e.offsetY;return"translate("+s+", "+r+")"})},Pattern.prototype.getStopData=function(){var t=[];return this.graphEdges.forEach(function(e,s){s>0?this.graphEdges[s-1]:null;var r,o=s<this.graphEdges.length-1?this.graphEdges[s+1]:null;if(0===s&&(r={x:e.fromVertex.x,y:e.fromVertex.y,stop:e.fromVertex.stop,inEdge:null,outEdge:e},r.offsetX=this.offset?e.rightVector.x*this.lineWidth*this.offset:0,r.offsetY=this.offset?e.rightVector.y*this.lineWidth*this.offset:0,t.push(r)),e.stopArray.forEach(function(s,o){r=e.pointAlongEdge((o+1)/(e.stopArray.length+1)),r.stop=s,r.inEdge=r.outEdge=e,this.offset?(r.offsetX=e.rightVector.x*this.lineWidth*this.offset,r.offsetY=e.rightVector.y*this.lineWidth*this.offset):r.offsetX=r.offsetY=0,t.push(r)},this),r={x:e.toVertex.x,y:e.toVertex.y,stop:e.toVertex.stop,inEdge:e,outEdge:null},this.offset)if(o&&o.rightVector.x!==e.rightVector.x&&o.rightVector.y!==e.rightVector.y){var i={x:o.rightVector.x+e.rightVector.x,y:o.rightVector.y+e.rightVector.y},h=Math.sqrt(i.x*i.x+i.y*i.y),n={x:i.x/h,y:i.y/h},a=Math.sqrt(Math.pow(o.rightVector.x-e.rightVector.x,2)+Math.pow(o.rightVector.y-e.rightVector.y,2))/2,f=1/Math.sqrt(1-a*a);r.offsetX=n.x*this.lineWidth*this.offset*f,r.offsetY=n.y*this.lineWidth*this.offset*f}else r.offsetX=e.rightVector.x*this.lineWidth*this.offset,r.offsetY=e.rightVector.y*this.lineWidth*this.offset;else r.offsetX=r.offsetY=0;t.push(r)},this),t},Pattern.prototype.getGraphVertices=function(){var t=[];return this.graphEdges.forEach(function(e,s){0===s&&t.push(e.fromVertex),t.push(e.toVertex)},this),t};
});
...
function StyleSheet(e,t){return this instanceof StyleSheet?(this.vars={},this.rules={},this.elem=null,e&&this.add(e),t&&this.define(t),this.load(),void 0):new StyleSheet(e,t)}function createStyleSheetElement(){var e=document.createElement("style"),t=document.getElementsByTagName("head")[0];return t.appendChild(e),e}function refresh(e,t,r){var i="";for(var n in t){i+=n+"{";for(var s in t[n])i+=s+":"+substitute(t[n][s],r)+";";i+="}"}e.innerHTML=i}function substitute(e,t){for(var r in t)e=e.replace(new RegExp("@"+r,"gi"),t[r]);return e}module.exports=StyleSheet,StyleSheet.prototype.define=function(e){for(var t in e)this.vars[t]=e[t];return this},StyleSheet.prototype.add=function(e){for(var t in e){this.rules[t]||(this.rules[t]={});for(var r in e[t])this.rules[t][r]=e[t][r]}return this},StyleSheet.prototype.load=function(){return this.elem||(this.elem=createStyleSheetElement()),refresh(this.elem,this.rules,this.vars),this},StyleSheet.prototype.clear=function(){return this.elem.innerHTML="",this.rules="",this.vars={},this},StyleSheet.prototype.remove=function(){var e=this.elem;return e&&e.parentNode&&(this.clear(),this.elem=null,e.parentNode.removeChild(e)),this};
});
...
function Transitive(t,r,e,s){return this instanceof Transitive?(this.display=new Display(t),this.display.zoom.on("zoom",this.refresh.bind(this)),this.graph=new Graph,this.style=new Styler(e,s),this.load(r),this.render(),void 0):new Transitive(t,r,e,s)}function generateStops(t){var r={};return t.forEach(function(t){r[t.stop_id]=new Stop(t)}),r}function populateGraphEdges(t,r){var e=null,s=[];for(var i in t){var a=t[i];for(var n in a.stops){var p=a.stops[n];if(p.graphVertex){if(null!==e){var o=r.getEquivalentEdge(s,e,p.graphVertex);o||(o=r.addEdge(s,e,p.graphVertex)),a.graphEdges.push(o)}e=p.graphVertex,s=[]}else s.push(p)}}}var Display=require("display"),Graph=require("graph"),Pattern=require("pattern"),Route=require("route"),Stop=require("stop"),Styler=require("styler");module.exports=Transitive,Transitive.prototype.load=function(t){this.stops=generateStops(t.stops),this.routes={},this.patterns={};var r={};t.routes.forEach(function(t){var e=new Route(t);this.routes[e.route_id]=e,t.patterns.forEach(function(t){var s=new Pattern(t);this.patterns[t.pattern_id]=s,e.addPattern(s),t.stops.forEach(function(t){var r=this.stops[t.stop_id];s.stops.push(r),r.patterns.push(s)},this);var i=s.stops[0];i.getId()in r||(r[i.getId()]=i);var a=s.stops[s.stops.length-1];a.getId()in r||(r[a.getId()]=a)},this)},this);for(var e in r){var s=r[e],i=this.graph.addVertex(s,0,0);s.graphVertex=i}populateGraphEdges(this.patterns,this.graph)},Transitive.prototype.render=function(){for(var t in this.patterns)this.patterns[t].draw(this.display,10);this.refresh()},Transitive.prototype.refresh=function(){for(var t in this.patterns){var r=this.patterns[t];this.style.render(r.svgGroup,this.display),r.refresh(this.display)}},Transitive.prototype.setElement=function(t){this.display.setElement(t),this.render()};
});
