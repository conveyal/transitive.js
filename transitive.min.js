...
function Display(t){this.offsetLeft=t.offsetLeft,this.offsetTop=t.offsetTop,this.labelZoomThreshold=.75,this.xScale=d3.scale.linear().domain([-2,2]),this.yScale=d3.scale.linear().domain([-2,2]),this.zoom=d3.behavior.zoom().scaleExtent([.25,4]),this.svg=d3.select(t).append("svg").append("g").call(this.zoom),this.svg.append("rect").attr("class","overlay"),this.setElement(t)}var d3=require("d3");module.exports=Display,Display.prototype.setElement=function(t){var e=t.clientWidth,s=t.clientHeight;this.xScale.range([0,e]),this.yScale.range([s,0]),this.zoom.x(this.xScale).y(this.yScale),this.svg.attr("width",e).attr("height",s),this.svg.select("rect").attr("width",e).attr("height",s)};
});
