...
NetworkPath.prototype.draw = function(display, capExtension) {
...
NetworkPath.prototype.refresh = function(display, styler) {
  // compute the line width
  var lw = styler.compute(styler.patterns['stroke-width'], display, this);
  this.lineWidth = parseFloat(lw.substring(0, lw.length - 2), 10) - 2;

  // update the line and stop groups
  for(var i = 0; i < this.segments.length; i++) {
    var segment = this.segments[i];
    segment.lineGraph.attr('d', this.line(segment.renderData));
  }
};
...
NetworkPath.prototype.refreshRenderData = function() {
  ...
  this.graphEdges.forEach(function (edgeInfo, i) {
    ...
    edge.pointArray.forEach(function (point, i) {
      pointInfo = edge.pointAlongEdge((i + 1) / (edge.pointArray.length + 1));
      pointInfo.path = this;
    ...
    this.segments[edgeIndex].renderData = edgeRenderData;
    edgeIndex++;
