...
exports.stops = {
  ...
  r: [
    ...
    function (display, data, index, utils) {
      if (data.stop.isEndPoint) {
        var width = data.stop.renderData.length * utils.strokeWidth(display) / 2;
  ...
  'stroke-width': [
    ...
    function (display, data, index, utils) {
      if (data.stop.isEndPoint) {
        return data.stop.renderData.length * utils.strokeWidth(display) / 2 + 'px';
      }
    }
  ],
...
exports.labels = {
  ...
  visibility: [
    ...
    function (display, data) {
      ...
      if (display.zoom.scale() >= 0.6 && data.stop && data.stop.isBranchPoint) return 'visible';
      if (display.zoom.scale() >= 0.4 && data.stop && data.stop.isEndPoint) return 'visible';
    }
  ],
  x: [ function (display, data, index, utils) {
    ...
    return Math.sqrt(width * width * 2) * (data.stop ? data.stop.labelPosition : 1) + 'px';
  }],
  y: [ function (display, data, index, utils) {
    return utils.fontSize(display, data) / 2 * -(data.stop ? data.stop.labelPosition : 1) + 'px';
  }],
  'text-transform': [
    'capitalize',
    function (display, data) {
      if (data.stop && data.stop.isEndPoint) return 'uppercase';
    }
  ]
};
