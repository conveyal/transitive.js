...
exports.labels = {
  ...
  x: [ function (display, data, index, utils) {
    ...
    return data.point.labelPosition * 10;
  }],
  y: [ function (display, data, index, utils) {
    //return utils.fontSize(display, data) / 2 * -(data.point ? data.point.labelPosition : 1) + 'px';
    return utils.fontSize(display, data) / 2 + 'px';
  }],
  'text-transform': [
    'capitalize',
    function (display, data) {
      if (data.point && (data.point.isSegmentEndPoint || data.point.containsToPlace || data.point.containsFromPlace)) return 'uppercase';
    }
  ]
};
