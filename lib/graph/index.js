...
NetworkGraph.prototype.apply2DOffsets = function(transitive) {
  ...
  for(var gridEdgeId in gridEdgeSegments) {
    ...
    for(var i =0; i < gridSegments.length; i++) {
      ...
      if(axisSegments.indexOf(segment) === -1) axisSegments.push(segment);
    }
  }
  ...
  for(var axisId in axisBundles) {
    ...
    for(var s = 0; s < sortedSegments.length; s++) {
      ...
      transitive.offsetSegment(sortedSegments[s], axisId, offset);
    }
  }
};
...
NetworkGraph.prototype.calculateGeometry = function() {
  this.edges.forEach(function(edge) {
...
NetworkGraph.prototype.optimizeCurvature = function() {
  ...
  // optimize neighbors of axial edges first
  this.edges.forEach(function(edge) {
    if(edge.isAxial()) {
      edge.pathSegments.forEach(function(segment) {
        if(segment.getType() === 'TRANSIT') {
          this.alignIncidentEdges(edge.fromVertex, edge, segment.pattern);
          this.alignIncidentEdges(edge.toVertex, edge, segment.pattern);
        }
      }, this);
    }
  }, this);
...
NetworkGraph.prototype.alignIncidentEdges = function(vertex, inEdge, pattern) {
  vertex.incidentEdges(inEdge).forEach(function(edge) {
    if(edge.aligned) return;
    var segment = edge.renderSegment;
    if(segment.getType() === 'TRANSIT' && segment.pattern === pattern) {
      var inEdgeVector = inEdge.getVector(vertex);
      edge.align(vertex, {
        x: -inEdgeVector.x,
        y: -inEdgeVector.y
      });
