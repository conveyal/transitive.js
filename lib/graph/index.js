...
NetworkGraph.prototype.initComparisons = function() {
  ...
  each(this.vertices, function(vertex) {
    ...
    each(edges, function(edge) {
      ...
      angleSegments[angleDeg] = angleSegments[angleDeg].concat(edge.renderSegments);
    });
    ...
    each(angleSegments, function(angle) {
      ...
      for (var i = 0; i < segments.length - 1; i++) {
        for (var j = i + 1; j < segments.length; j++) {
          ...
          //var outVector1 = getOutVector(s1.graphEdge, vertex);
          //var outVector2 = getOutVector(s2.graphEdge, vertex);
          ...
          //var isOutward = Util.isOutwardVector(outVector1) && Util.isOutwardVector(outVector2);
...
function getInverse(s1, s2, vertex) {
  return ((s1.graphEdge.toVertex === vertex && s2.graphEdge.toVertex === vertex) || 
          (s1.graphEdge.toVertex === vertex && s2.graphEdge.fromVertex === vertex)) ? -1 : 1;
}
