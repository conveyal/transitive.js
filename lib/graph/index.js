...
NetworkGraph.prototype.bounds = function() {
  ...
  for (var i in this.vertices) {
    ...
    if (vertex.point.getLat() !== 0 && vertex.point.getLon() !== 0) {
      lat.min = Math.min(lat.min, vertex.point.getLat());
      lat.max = Math.max(lat.max, vertex.point.getLat());
      lon.min = Math.min(lon.min, vertex.point.getLon());
      lon.max = Math.max(lon.max, vertex.point.getLon());
    }
  }
...
NetworkGraph.prototype.splitEdgeAtInternalPoints = function(edge, points) {
  ...
  each(edge.pointArray, function(point) {
    if(points.indexOf(point) !== -1) {
      ...
      newEdges.push(this.addEdge(subEdgePoints, fromVertex, newVertex));
      subEdgePoints = [];
  ...
  newEdges.push(this.addEdge(subEdgePoints, fromVertex, edge.toVertex));
  ...
  each(this.vertices, function(vertex) {
    ...
    for(var i = 0; i < edges.length-1; i++) {
      for(var j = i+1; j < edges.length; j++) {
        ...
        //console.log('OVs:');
        //console.log(outVector1);
        //console.log(outVector2);
...
NetworkGraph.prototype.recenter = function() {
  ...
  var mx = d3.median(xCoords), my = d3.median(yCoords);
