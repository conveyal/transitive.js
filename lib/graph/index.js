...
NetworkGraph.prototype.addEdge = function(stops, from, to) {
  ...
  var groupKey = this.getEdgeGroupKey(edge);

  if(!(groupKey in this.edgeGroups)) this.edgeGroups[groupKey] = new EdgeGroup(edge.fromVertex, edge.toVertex);
...
NetworkGraph.prototype.getEdgeGroupKey = function(edge) {
  return edge.fromVertex.getId() < edge.toVertex.getId() ?
    edge.fromVertex.getId() + '_' + edge.toVertex.getId() :
    edge.toVertex.getId() + '_' + edge.fromVertex.getId();
};
...
function EdgeGroup(fromVertex, toVertex) {
