...
Edge.prototype.getWorldLength = function() {
  if(!this.worldLength) this.calculateWorldLength();
  return this.worldLength;
};


Edge.prototype.calculateWorldLength = function() {
  ...
  each(this.pointArray, function(point) {
    ...
    this.worldLength += Util.distance(x1, y1, x2, y2);
...
Edge.prototype.calculateGeometry = function(cellSize, angleConstraint) {
  ...
  var tol = 0.01;
  var v = Util.normalizeVector({ x : (this.toVertex.x - this.fromVertex.x), y : (this.toVertex.y - this.fromVertex.y) });

  // check if we need to add curvature
  if (!equalVectors(fvx, fvy, -tvx, -tvy, tol) || !equalVectors(fvx, fvy, v.x, v.y, tol)) {

    // if so, adjust the two component segments until they properly intersect
