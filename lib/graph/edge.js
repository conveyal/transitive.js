...
Edge.prototype.calculateGeometry = function(cellSize, angleConstraint) {
  ...
  var fx = this.fromVertex.point.worldX, fy = this.fromVertex.point.worldY;
  var tx = this.toVertex.point.worldX, ty = this.toVertex.point.worldY;

  var fromAdjPoint = this.getAdjPoint(this.fromVertex.point); // this.pointArray.length > 0 ? this.pointArray[0] : this.toVertex.point;
  
  //console.log(' F: ' + this.fromVertex.point.getName());
  var dx = fromAdjPoint.worldX - fx;
  var dy = fromAdjPoint.worldY - fy;

  var fromAngle = Util.getVectorAngle(dx, dy);// * 180 / Math.PI;
  var constrainedFromAngle = Math.round(fromAngle/angleConstraintR) * angleConstraintR;
  var fvx = Math.cos(constrainedFromAngle), fvy = Math.sin(constrainedFromAngle);
  //console.log('  ' + dx + ', ' + dy + ' | a='+fromAngle + ' -> '+(constrainedFromAngle * 180/Math.PI));
  //console.log('  fv: ' + fvx + ', '+ fvy);
  ...
  //console.log(' T: ' + this.toVertex.point.getName());
  dx = toAdjPoint.worldX - tx;
  dy = toAdjPoint.worldY - ty;

  var toAngle = Util.getVectorAngle(dx, dy);// * 180 / Math.PI;
  var constrainedToAngle = Math.round(toAngle/angleConstraintR) * angleConstraintR;
  var tvx = Math.cos(constrainedToAngle), tvy = Math.sin(constrainedToAngle);
  //console.log('  ' + dx + ', ' + dy + ' | a='+toAngle + ' -> '+(constrainedToAngle * 180/Math.PI));
  //console.log('  tv: ' + tvx + ', '+ tvy);

  var tol = 0.00001;
  var s1 = (this.toVertex.y - this.fromVertex.y) / (this.toVertex.x - this.fromVertex.x);
  var s2 = fvy/fvx;
  //console.log('  s: '+ s1 + ' / ' + s2);
...
Edge.prototype.getAdjPoint = function(point) {
  if(point === this.fromVertex.point) return this.pointArray.length > 0 ? this.pointArray[0] : this.toVertex.point;
  if(point === this.toVertex.point) return this.pointArray.length > 0 ? this.pointArray[this.pointArray.length-1] : this.fromVertex.point;
};
...
Edge.prototype.getRenderCoords = function(fromOffsetPx, toOffsetPx, display) {
  ...
  if(!this.isStraight()) {
    ...
    if(isect.intersect) {
      ...
      var l1 = Util.distance(fx, fy, ex, ey), l2 = Util.distance(tx, ty, ex, ey);
      var rPx = this.getBaseRadiusPx() - (fromOffsetPx + toOffsetPx) / 2;
      rPx = Math.min(Math.min(l1, l2), rPx);
...
Edge.prototype.coordAlongEdge = function(t, coords, display) {
  ...
  for(i=1; i < coords.length; i++) {
    if(target <= coveredLen + coords[i].len) { // this pair of coords 'contains' the target
      if(coords[i].arc) {
        return null;
      }
      else {
        ...
        var t2 = (target - coveredLen) / coords[i].len;
        return {
          x : coords[i-1].x + t2 * dx,
          y : coords[i-1].y + t2 * dy
        };
      }

    }
    coveredLen += coords[i].len;
  }

};


Edge.prototype.coordAlongEdge_old = function(t, coords, display) {
  ...
  if(this.baseCoords.length !== coords.length) {
    console.log('coordAlongEdge error');
    console.log(this.baseCoords);
    console.log(coords);
  }
