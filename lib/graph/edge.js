...
Edge.prototype.getRenderCoords = function(fromOffsetPx, toOffsetPx, display) {
  ...
  if(!this.isStraight()) {
    ...
    if(isect.intersect) {
      ...
      var x1 = ex - this.fromVector.x * d;
      var y1 = ey + this.fromVector.y * d;
      coords.push({
        x: x1,
        y: y1,
      ...
      var x2 = ex + this.toVector.x * d;
      var y2 = ey - this.toVector.y * d;
...
Edge.prototype.coordAlongEdge = function(t, coords, display, dbg) {
  ...
  for(i = 1; i < this.baseCoords.length; i++) {
    if(loc < cur + this.baseCoords[i].len) {
      ...
      if(coords[i].arc) {
        ...
        vectFromCenter = Util.normalizeVector(Util.rotateVector(vectFromCenter, rot));
