...
export default function drawGrid (display, cellSize) {
  ...
  for (var x = xMin; x <= xMax; x += cellSize) appendLine(xScale.compute(x), xScale.compute(x), yRange[0], yRange[1])

  var yMin = Math.round(yDomain[0] / cellSize) * cellSize
  var yMax = Math.round(yDomain[1] / cellSize) * cellSize
  for (var y = yMin; y <= yMax; y += cellSize) appendLine(xRange[0], xRange[1], yScale.compute(y), yScale.compute(y))
}
