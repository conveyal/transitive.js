...
RenderSegment.prototype.refreshRenderData = function(display) {
  ...
  /*if(this.path.segments.length === 1) {
    console.log('rs for single segment path');
    console.log(' ' + this.getType());
    console.log(' ' + this.graphEdge.toString());
  }*/
  
  /*if (this.getType() === 'WALK' && this.path.segments.length > 1) {
...
RenderSegment.prototype.getLabelAnchors = function(display) {
  ...
  if (this.renderData.length === 2) { // basic straight segment
    var dx = this.renderData[1].x - this.renderData[0].x;
    var dy = this.renderData[1].y - this.renderData[0].y;

    var getAnchor = (function(t) {
      return {
        x: this.renderData[0].x + dx * t,
        y: this.renderData[0].y + dy * t
      };
    }).bind(this);

    labelAnchors.push(getAnchor(0.5));
    labelAnchors.push(getAnchor(0.33));
    labelAnchors.push(getAnchor(0.66));
  }
