...
RenderSegment.prototype.refreshRenderData = function(display) {
  ...
  if(this.getType() === 'WALK') {
    ...
    this.renderData = this.graphEdge.getRenderCoords(fromOffsetPx, toOffsetPx, display, (this.id === 1));
  }
...
RenderSegment.prototype.intersect = function(segment) {
  ...
  var p1 = (commonVertex === this.graphEdge.fromVertex) ? this.renderData[0] : this.renderData[this.renderData.length-1];
  var v1 = this.graphEdge.getVector(commonVertex);

  var p2 = (commonVertex === segment.graphEdge.fromVertex) ? segment.renderData[0] : segment.renderData[segment.renderData.length-1];
