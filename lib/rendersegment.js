...
RenderSegment.prototype.addPattern = function(pattern) {
...
RenderSegment.prototype.addPathSegment = function(pathSegment) {
...
function constuctIdListString(items) {
  var idArr = [];
  each(items, function(item) { idArr.push(item.getId()); });
  idArr.sort();
  return idArr.join(',');
}
...
RenderSegment.prototype.getAlignmentVector = function(alignmentId) {
...
RenderSegment.prototype.findExtension = function(vertex) {
  ...
  for(var e = 0; e < incidentEdges.length; e++) {
    var edgeSegments = incidentEdges[e].renderSegments;
...
RenderSegment.prototype.getLabelAnchors = function(display, spacing) {
...
RenderSegment.prototype.getLabelTextArray = function() {
  var textArray = [];
...
RenderSegment.prototype.toString = function() {
  return 'RenderSegment ' + this.id + ' type=' + this.type + ' on ' + this.graphEdge.toString();
};