...
Transitive.prototype.sortElements = function(journeyId) {
  this.display.svg.selectAll('.transitive-sortable').sort(function(a, b) {
    ...
    // if comparing a focused item and non-focused item, render the focused item in front
    if(a.focused === true && b.focused === false) return 1;
    if(b.focused === true && a.focused === false) return -1;

    // if comparing a label and something else, show the label in front
    if(a.sortableType === 'LABEL' && b.sortableType !== 'LABEL') return 1;
    if(b.sortableType === 'LABEL' && a.sortableType !== 'LABEL') return -1;

    // if comparing a point and something else, show the point in front
    if(a.sortableType === 'POINT' && b.sortableType !== 'POINT') return 1;
    if(b.sortableType === 'POINT' && a.sortableType !== 'POINT') return -1;

    // if comparing two segments, use internal comparison function
    if(a.sortableType === 'SEGMENT' && b.sortableType === 'SEGMENT') {
