...
Transitive.prototype.createRenderSegment = function(pathSegment, edge, patterns) {
  ...
  if (key in this.rsLookup) {
    ...
    renderSegment.addPathSegment(pathSegment);
...
Transitive.prototype.refreshSegmentRenderData = function() {
  ...
  each(this.patterns, function(patternId) {
  ...
  each(this.paths, function(path) {
    each(path.segments, function(pathSegment) {
      if(pathSegment.type !== 'WALK' && pathSegment.type !== 'BICYCLE' && pathSegment.type !== 'CAR') return;
      for (var s = 0; s < pathSegment.renderSegments.length - 1; s++) {
        var seg1 = pathSegment.renderSegments[s];
        var seg2 = pathSegment.renderSegments[s + 1];
        if (seg1.graphEdge.isInternal && seg2.graphEdge.isInternal) {
          seg1.intersect(seg2);
        }
      }
