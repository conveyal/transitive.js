...
Transitive.prototype.load = function(data) {
  ...
  applyFilters(data.patterns, this._filter.patterns).forEach(function (data) {
    this.patterns[data.pattern_id] = new RoutePattern(data, this);
  }, this);
  ...
  for(var i = 0; i < this.vertexPoints.length; i++) {
    ...
    var vertex = this.graph.addVertex(point, 0, 0);
...
Transitive.prototype.render = function() {
  ...
  // initialize the path svg elements
  for(var p = 0; p < this.paths.length; p++) {
    var path = this.paths[p];
    path.refreshRenderData();
    path.draw(this.display, 0); // 10);
  }
...
Transitive.prototype.refresh = function() {
  ...
  for(p = 0; p < this.paths.length; p++) {
    var path = this.paths[p];
    path.refreshRenderData(); // also updates the stop-level renderData

    for(var s = 0; s < path.segments.length; s++) {
      this.style.renderPattern(this.display, path.segments[s].lineGraph);
    }

    path.refresh(this.display, this.style);
  }
