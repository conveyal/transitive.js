...
Transitive.prototype.updateGeometry = function(snapGrid) {
  ...
  //this.placePointLabels();
  //this.labeler.doLayout();
};
...
Transitive.prototype.render = function() {
  ...
  for (var key in this.stops) this.stops[key].draw(this.display);

  // Draw the place svg elements
  for (key in this.places) this.places[key].draw(this.display);
  */
...
Transitive.prototype.refresh = function() {
  ...
  for (key in this.places) {
    ...
    if (!place.svgGroup) continue; // check if this place is not currently rendered

    this.style.renderPlace(this.display, place);
    place.refresh(this.display);
  }*/
  ...
  var labeledPoints = this.labeler.doLayout();
  labeledPoints.forEach(function(point) {
    point.refreshLabels(this.display, 0 , 0);
...
Transitive.prototype.populateRenderSegments = function() {
  ...
  /*this.graph.edges.forEach(function(edge) {
    console.log(' ');
    console.log(edge.fromVertex.point.getName() + ' to ' + edge.toVertex.point.getName());

    edge.pathSegments.forEach(function(segment) {
      if(segment.pattern) console.log('- ' + segment.pattern.getName() + ' / ' + segment.pattern.getId());
      var renderSegment = new Segment(segment.type);
      renderSegment.pattern = segment.pattern;
      renderSegment.addEdge(edge);
      renderSegment.points.push(edge.fromVertex.point);
      renderSegment.points.push(edge.toVertex.point);
      edge.addRenderSegment(renderSegment);
...
Transitive.prototype.placePointLabels = function() {

  var gridPointSegments = { }, gridPointLabels = { };

  this.graph.edges.forEach(function(edge) {
    edge.gridPoints.forEach(function(gridPoint) {
      var gridPointId = gridPoint[0] + '_' + gridPoint[1];

      if(!(gridPointId in gridPointSegments)) {
        gridPointSegments[gridPointId] = [];
      }

      var segmentList = gridPointSegments[gridPointId];
  ...
  this.graph.orderedVertices.forEach(function(vertex) {
    var point = vertex.point;
    if(point.getType() === 'PLACE' || point.getType() === 'MULTI' || (point.getType() === 'STOP' && point.isSegmentEndPoint)) {
      this.placeLabel(vertex, point, gridPointSegments, gridPointLabels);
    }
  }, this);
};


Transitive.prototype.placeLabel = function(vertex, point, gridPointSegments, gridPointLabels) {
  var cellSize = this.gridCellSize;
  var axialCount = 4, diagonalCount = 3;

  // try right
  if(tryLabelPlacement(vertex, point, cellSize, 0, axialCount, 0, 1, gridPointSegments, gridPointLabels)) return;

  // try left
  if(tryLabelPlacement(vertex, point, -cellSize, 0, axialCount, 0, -1, gridPointSegments, gridPointLabels)) return;

  // try upper-right
  if(tryLabelPlacement(vertex, point, cellSize, cellSize, diagonalCount, -45, 1, gridPointSegments, gridPointLabels)) return;
  ...
  // try vertical above
  if(tryLabelPlacement(vertex, point, 0, cellSize, axialCount, -90, 1, gridPointSegments, gridPointLabels)) return;
...
function tryLabelPlacement(vertex, point, dx, dy, count, angle, position, gridPointSegments, gridPointLabels) {
  var gridPointIds = getLabelGridPoints(vertex, dx, dy, count);
  var score = getLabelGridPointScore(gridPointIds, gridPointSegments, gridPointLabels);
  if(score === 0) {
    point.updateLabelPosition(angle, position);

    gridPointIds.forEach(function(gridPointId) {
      if(!(gridPointId in gridPointLabels)) {
        gridPointLabels[gridPointId] = [];
      }
      gridPointLabels[gridPointId].push(point);
    });
...
function getLabelGridPoints(vertex, dx, dy, count) {
  var x0 = vertex.x, y0 = vertex.y;
  var gridPointIds = [];
  for(var i = 0; i < count; i++) {
    var x1 = x0 + dx, y1 = y0 + dy;
    gridPointIds.push(x1 + '_' + y1);
    x0 = x1;
    y0 = y1;
  }
  return gridPointIds;
...
function getLabelGridPointScore(gridPointIds, gridPointSegments, gridPointLabels) {
  var score = 0;
  for(var i = 0; i < gridPointIds.length; i++) {

    var segments = gridPointSegments[gridPointIds[i]];
    var segLen = segments ? segments.length : 0;

    var labels = gridPointLabels[gridPointIds[i]];
    var labLen = labels ? labels.length : 0;

    score += (segLen + 5*labLen);
  }
  return score;
}
