...
function Transitive(options) {
  ...
  this.zoomFactors = this.options.zoomFactors || this.getDefaultZoomFactors();
...
Transitive.prototype.getDefaultZoomFactors = function(data) {
  return [
    {
      minScale: 0,
      gridCellSize: 100,
...
Transitive.prototype.load = function(data) {
  ...
  this.updateZoomFactors(this.zoomFactors[0]);
  this.createGraph();
...
Transitive.prototype.createGraph = function() {
  ...
  this.graph = new Graph(this, this.baseVertexPoints);
  this.populateGraphEdges();
  this.graph.collapseTransfers(this.collapseTransferThreshold);
...
Transitive.prototype.updateZoomFactors = function(factors) {
  ...
  this.collapseTransferThreshold = factors.collapseTransferThreshold;
};
  ...
  each(this.paths, function(path) {
    each(path.segments, function(segment) {
      ...
      if(segment.geomCoords && this.internalVertexFactor <= 1) {
        var edge = this.graph.addEdge(internalPoints, segment.points[0].graphVertex, segment.points[segment.points.length-1].graphVertex, segment.getType());
    ...
    function() {
      ...
      if (scale !== self.lastScale) { // zoom action
        ...
        for(var i=0; i < self.zoomFactors.length; i++) {
          ...
          if((self.lastScale < min || self.lastScale >= max) && scale >= min && scale < max) {
            self.updateZoomFactors(self.zoomFactors[i]);
            self.clearGraphData();
            self.createGraph();
            self.render();
            updatedGraph = true;
...
Transitive.prototype.render = function() {
  ...
  // draw the segments
  /*for (var s = 0; s < this.renderedEdges.length; s++) {
    var rEdge = this.renderedEdges[s];
    rEdge.refreshRenderData(this.display);
    rEdge.render(this.display, 0); // 10);
    var legendType = rEdge.getLegendType();
    if (!(legendType in legendSegments)) {
      legendSegments[legendType] = rEdge;
    }
  }*/
...
Transitive.prototype.sortElements = function() {
  ...
  this.graph.vertices.forEach(function(vertex) {
    var point = vertex.point;
    point.zIndex = 10000 + (point.isFocused() ? focusBaseZIndex : 0);
  }, this);
...
Transitive.prototype.focusJourney = function(journeyId) {
  ...
  var focusChangeSegments = []; //, defocusedSegments = [];
  ...
  this.renderedSegments.forEach(function(rSegment) {
    if(!journeyId || journeyRenderedSegments.indexOf(rSegment) !== -1) {
      each(rSegment.renderedEdges, function(rEdge) {
        focusedVertexPoints.push(rEdge.graphEdge.fromVertex.point);
        focusedVertexPoints.push(rEdge.graphEdge.toVertex.point);
        /*if(segment.pattern) {
