      ...
      each(pathSegment.graphEdges, function(edge) {
        ...
        if(pathSegment.type === 'TRANSIT') {
          each(pathSegment.getPatterns(), function(pattern) {
            this.createRenderSegment(pathSegment, edge, pattern);
          }, this);
        }
...
Transitive.prototype.createRenderSegment = function(pathSegment, edge, pattern) {
  var renderSegment;
  var key = edge.id + '_' + pathSegment.getType() + (pattern ?
    '_' + pattern.pattern_id : '');
