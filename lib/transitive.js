...
Transitive.prototype.createRenderSegment = function(pathSegment, edge, patterns) {
  ...
  if(patterns) {
    var patternIdArr = [];
    each(patterns, function(pattern) { patternIdArr.push(pattern.getId()); });
    patternIdArr.sort();
    patternIds = patternIdArr.join(',');
    key += '_' + patternIds;
  }
