...
Transitive.prototype.load = function(data) {
  ...
  if(data.routes) {
    applyFilters(data.routes, this._filter.routes).forEach(function (routeData) {
      ...
      applyFilters(routeData.patterns, this._filter.patterns).forEach(function (patternData, i) {
        var pattern = this.processStopSequence(patternData, vertexStops, adjacentStops);
        pattern.route = route;
      }, this);
    }, this);
  }
  ...
  if(data.journeys) {
    data.journeys.forEach((function(journeyData) {
      this.processStopSequence(journeyData, vertexStops, adjacentStops);
    }).bind(this));
  }
...
Transitive.prototype.processStopSequence = function(patternData, vertexStops, adjacentStops) {
...
Transitive.prototype.loadProfile = function(profileData, endpoint, callback, maxOptions) {
  var profile = new OtpProfile(profileData, endpoint, (function(data) {
    this.data = data;
    callback.call(this, data);
  }).bind(this), maxOptions);
