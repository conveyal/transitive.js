...
RenderedSegment.prototype.refresh = function(display, renderData) {
  ...
  //var dbg = this.pathSegment && this.pathSegment.type === 'TRANSIT';
  //var dbg = this.pathSegment && this.pathSegment.startVertex().point.getId() === 'DC:5017';
  var dbg = false;

  if(dbg) {
    console.log('RS ref ' + this.toString());
    console.log(this.pathSegment.getPatterns());
  }
  ...
  if (renderData) {
    ...
    each(this.renderedEdges, function(rEdge) {
      if(dbg) console.log(' - ' + rEdge.toString());
