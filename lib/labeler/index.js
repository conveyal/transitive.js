...
var Labeler = augment(Object, function() {
  ...
  this.placeSegmentLabels = function() {
    ...
    each(this.segmentLabels, function(label) { label.clear(); });
    this.segmentLabels = [];

    // collect the RenderSegment sequences for the graph
    var sequences = [];
    each(this.transitive.paths, function(path) {
      each(path.segments, function(pathSegment) {
        // only consider bus path segments
        if(pathSegment.type !== 'TRANSIT' || pathSegment.getMode() !== 3) return;
        sequences = sequences.concat(pathSegment.getRenderSegmentSequences());
      });
    }, this);

    // iterate through the sequence collection, labeling as necessary
    var visitedRSegments = [];
    each(sequences, function(sequence) {
      // take the first rendersegment as representative of this sequence
      var rsegment = sequence.renderSegments[0];

      // don't label equivalent sequences multiple times
      if(visitedRSegments.indexOf(rsegment.id) !== -1) return;
      visitedRSegments.push(rsegment.id);

      // get the array of label strings to be places (typically the unique route short names)
      var labelTextArray = rsegment.getLabelTextArray();

      // create the initial label for placement
      var labelTextIndex = 0;
      var label = this.constructSegmentLabel(rsegment, labelTextArray[labelTextIndex]);

      // iterate through potential anchor locations, attempting placement at each one
      var labelAnchors = sequence.getLabelAnchors(this.transitive.display, label.textHeight*1.5);
      for(var i = 0; i < labelAnchors.length; i++) {
        ...
        if(conflicts.length === 0) { // if no conflicts
          ...
          // advance to next label in text array, if applicable
          if(labelTextIndex < labelTextArray.length-1) {
            labelTextIndex++;
            label = this.constructSegmentLabel(rsegment, labelTextArray[labelTextIndex]);
          }
          else break;
  ...
  this.placePointLabels = function() {
    ...
    this.points.forEach(function(point) {
      ...
      point.label.fontFamily = styler.compute(styler.labels['font-family'], this.transitive
        .display, {
          point: point
        });
      point.label.fontSize = styler.compute(styler.labels['font-size'], this.transitive
        .display, {
          point: point
        });
      var textBBox = Util.getTextBBox(labelText, {
  ...
  this.findNearbySegmentLabels = function(label, x, y, buffer) {
    ...
    this.quadtree.visit(function(node, x1, y1, x2, y2) {
      var p = node.point;
      if ((p) && (p[0] >= minX) && (p[0] < maxX) && (p[1] >= minY) && (p[
        1] < maxY) && (p[2].parent) && (label.parent.patternIds === p[2].parent.patternIds)) {
        matchItems.push(p[2]);
      }
      return x1 > maxX || y1 > maxY || x2 < minX || y2 < minY;
    });
