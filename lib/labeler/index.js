...
var Labeler = augment(Object, function() {
  ...
  this.placeSegmentLabels = function() {
    ...
    this.transitive.renderSegments.sort(function(a,b) {
      var lenDiff = b.graphEdge.getWorldLength() - a.graphEdge.getWorldLength();
      return lenDiff !== 0 ? lenDiff : b.getId() - a.getId();
    });

    this.transitive.renderSegments.forEach(function(segment) {
      if (segment.getType() === 'TRANSIT' && segment.mode === 3) {
        ...
        segment.label.computeContainerDimensions();

        var labelAnchors = segment.getLabelAnchors(this.transitive.display);
        var placedLabel = false;

        for(var i = 0; i < labelAnchors.length; i++) {
          segment.label.labelAnchor = labelAnchors[i];

          // do not consider this anchor if it is out of the display range
          if(!this.transitive.display.isInRange(segment.label.labelAnchor.x,
            segment.label.labelAnchor.y)) continue;

          // check for conflicts with existing placed elements
          var bbox = segment.label.getBBox();
          var conflicts = this.findOverlaps(segment.label, bbox);

          if(conflicts.length === 0) { // if no conflicts

            // check for proximity to existing labels for this segment
            var nearbyLabels = this.findNearbySegmentLabels(segment.label, segment.label.labelAnchor.x,
              segment.label.labelAnchor.y, 200);

            if(nearbyLabels.length === 0) { // if nothing nearby, then place at this anchor and break out of loop
              ...
              placedLabel = true;
              //debug('placing seg label at %s, %s', segment.label.labelAnchor.x, segment.label.labelAnchor.y);
              break;
            }
          }
        }
        if(!placedLabel) segment.label.clear();
      }
    }, this);
