...
var Stop = augment(Point, function(base) {
  ...
  this.getMarkerBBox = function() {
    ...
    //console.log('gMBB stop ' + this.getName());
    //console.log(this);
    if(this.patternMarkers) return this.patternMarkers.node().getBBox();
  };
  ...
  this.runFocusTransition = function(display, callback) {
    if (this.mergedMarker) {
      var newStrokeColor = display.styler.compute(display.styler.stops_merged.stroke, display, {owner: this});
